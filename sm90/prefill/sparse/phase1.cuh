...
CUTE_DEVICE void st_global_cs_128(float f0, float f1, float f2, float f3, void *dst_ptr) { // 内联汇编，一次性写入4个float32，不留缓存，用于快速保存输出
    asm volatile("st.weak.global.cs.v4.f32 [%0], {%1, %2, %3, %4};\n" // store.弱一致性（程序能够保证写入地址没有冲突），gloabal mem，Cache Streaming（流式缓存策略，即不留缓存），vector 4（即一次写入一个4元素向量），float32
                 :
                 : "l"(dst_ptr), //目标地址[%0]，"l"(dst_ptr)表示把dst_ptr转译为64位整型
                   "f"(f0), "f"(f1), "f"(f2), "f"(f3) //输入数组{%1, %2, %3, %4}
                );
}

CUTE_DEVICE
float2 __shfl_xor_sync_float2( // float2: 两个float32组成的结构体。这个函数用于一次性规约两组float32元素。也就是online softmax中的sum和max
    uint32_t mask, float2 value, int offset
) {
    float2 res;
    // 转译成long long是因为longlong是64字节，转换为double，效果应该一样。
...
CUTE_DEVICE
void tma_bulk_reduce_add(void const* src_ptr, void* dst_ptr, int32_t store_bytes) { // 等价于dst_ptr[i] += src_ptr[i]
    uint32_t smem_int_ptr  = cast_smem_ptr_to_uint(src_ptr);
    asm volatile("cp.reduce.async.bulk.global.shared::cta.bulk_group.add.f32 [%0], [%1], %2;\n" // []表示寻址，{}表示多寄存器打包，什么都不包表示立即数。
                     :
                     : "l"(dst_ptr), "r"(smem_int_ptr), "r"(store_bytes) // r: 表示32位整型。store_bytes: 一共需要操作多少比特，或者说待处理向量长度。
                     : "memory");
}
...
__device__ void KernelTemplate<D_QK, HAVE_TOPK_LENGTH>::devfunc(const SparseAttnFwdParams &params, const TMAParams &tma_params) {
    ...
    const int warpgroup_idx = cutlass::canonical_warp_group_idx(); // 获取当前线程所在的warp group编号，等价于 threadIdx.x / 128
    const int warp_idx = cutlass::canonical_warp_idx_sync(); // 获取当前线程warp编号，等价于 threadIdx.x / 32
    const int idx_in_warpgroup = threadIdx.x % 128; // 获取当前线程在warp_group中的编号

    // Define shared tensors
    extern __shared__ char wksp_buf[];
    SharedMemoryPlan &plan = *reinterpret_cast<SharedMemoryPlan*>(wksp_buf);
    Tensor sQ = make_tensor(make_smem_ptr(plan.q_o.q.data()), SmemLayoutQ{}); //在smem中开辟空间，Q是持久存在的，因此单独开于smem
    ...
    Tensor sS1 = make_tensor(make_smem_ptr(plan.s[0].data()), SmemLayoutS{}); // S=QK 是双缓冲的主要优化对象

    if (warp_idx == 0 && elect_one_sync()) { //全局初始化操作，每个block只需要1个线程进行操作即可
        // Prefetch TMA descriptors
        cute::prefetch_tma_descriptor(tma_params.tma_Q.get_tma_descriptor()); // tma预热，搬数据前将layout数据载入到l1 cache。
        cute::prefetch_tma_descriptor(&tma_params.tensor_map_O);

        // Initialize barriers
        plan.bar_q.init(1); // mbarrier（事物屏障）初始化，存储于smem中
        CUTE_UNROLL // 解卷，循环次数是代码常量，因此让编译器直接重复循环代码以常量（比如这里是2）次即可，免去了判断和累加过程
        for (int i = 0; i < 2; ++i) {
        ...
        fence_barrier_init(); // 避免其它线程抢跑，防止barrier还没初始化就被使用。
        //这句代码确保：刚才所有的初始化写入操作，对 Thread Block 内的所有线程全局可见之后，程序才会继续往下执行。
        //（紧接着外面就会有一个 __syncthreads()，确保所有人都在这里等包工头干完活再一起出发）。
    }

    __syncthreads();
    
    const int topk_length = HAVE_TOPK_LENGTH ? __ldg(params.topk_length + s_q_idx) : params.topk; 
    //如果topk值随q的变化而变化，则访问params.topk_length[s_q_idx]，否则直接使用标量params.topk作为topk。
    ...
    // 注意到host接口函数的KU_ASSERT(params.topk % (2*B_TOPK) == 0);   // To save some boundry checkings这一句，表明params.topk必被B_TOPK整除，因此可以采用最快的unsigned除法（无需考虑符号，速度最快）进行计算，避免浪费时间。
    // 而不规则topk只能通过ceil_div来计算块数量。
    if (warpgroup_idx == 0 || warpgroup_idx == 1) {
        ...
        if (warp_idx == 0 && elect_one_sync()) {
            ...
            )(_, _, q_h_idx, _0{}); // 定位gmem切片
            launch_tma_copy(tma_params.tma_Q, gQ, sQ, plan.bar_q, TMA::CacheHintSm90::EVICT_FIRST); // tma内存搬运，注意这里有一个参数是plan.bar_q，这是为下一句奠定基础，它创造了TMA与plan.bar_q这段内存的联系
            plan.bar_q.arrive_and_expect_tx(B_H*D_Q*sizeof(bf16)); // mbarrier，专为async proxy设计（可以同步非CUDA Core中的线程），相当于一个支持自定义目标内存的fence，约束的是内存可见性。
            //B_H*D_Q*sizeof(bf16)表示需要有这么多内存写入方可读取，TMA的所有载入操作都会在plan.bar_q上登记。
        }

        float rM[2] = {MAX_INIT_VAL, MAX_INIT_VAL}; // Meaning: the `max_logits` used for O / rL calculation
        float rL[2] = {0.0f, 0.0f}; // sum初始化于register中
        Tensor rO = partition_fragment_C(TiledMMA_PV_LocalP{}, Shape<Int<B_H>, Int<D_V/2>>{}); // 寄存器布局初始化，用于存储计算结果 O 的一半 (一个warpgroup一半)
        Tensor rP = partition_fragment_C(TiledMMA_QK{}, Shape<Int<B_H>, Int<B_TOPK>>{}); // P即QK，未经softmax归一化
        Tensor rS = make_tensor<bf16>(partition_shape_A(TiledMMA_PV_LocalP{}, Shape<Int<B_H>, Int<B_TOPK>>{})); // S是softmax归一化后的QK
        cute::fill(rO, 0.0f); // 对rO初始化
        
        // Wait for Q
        plan.bar_q.wait(0); // 等待plan.bar_q.arrive_and_expect_tx(B_H*D_Q*sizeof(bf16));计数完毕
        ...
        auto qkt_gemm_one_tile = [&](auto warpgroup_idx, int tile_idx, bool clear_accum) {
            ...
            Tensor sK_tile = make_tensor(make_smem_ptr(plan.k[(int)IS_WG1].data() + tile_idx*B_TOPK*64), SmemLayoutKTiles<1>{}); // plan.k[(int)IS_WG1].data()，如果是WG0则调用k[0]，是WG1则调用k[1]
        ...
        auto mask_rP = [&](auto warpgroup_idx) {
            ...
            CUTE_UNROLL
            for (int row_idx = 0; row_idx < 2; ++row_idx) { // C 是分散存储在各个线程的寄存器里的(详见tensor core accumulator寄存器布局),在当前布局(2*2*8)下，每个线程存2行，并且连续2个一组，每组对应列间距为8
                CUTE_UNROLL
                for (int i = row_idx*2; i < size(rP); i += 4) { // 因为在regsiter中数据是按纵向排列的，但这里希望按行取，所以每次必须跳2行*2列个数据，或者说，跳过第二行的数据
                    int col = 8*(i/4) + (idx_in_warpgroup%4)*2; // 因为gmem坐标下，每隔8个，数据分布又回到了当前线程的寄存器里，所以这里要有一个8作为跳步因子，保证每次访问，对应数据都能在当前线程寄存器中找到。
                    // 8*(i/4)算出沿C维度，也就是循环载入的维度，的块偏移（块宽度为8，每次循环跳8步），(idx_in_warpgroup%4)*2 则算出块内偏移，*2是因为寄存器数据分配是两两一组的
        ...
        auto reduce_L = [&]() {
            ...
            NamedBarrier::arrive_and_wait(256, NamedBarriers::sL_ready); // barrier_id = NamedBarriers::sL_ready（即4，详见config.h），num_threads = 256
            // 通过命名屏障进行同步，注意，调用的是NamedBarrier，NameBarrier是cutlass提供的内联汇编接口类。
            // 其中arrive_and_wait函数的汇编代码是：asm volatile("bar.sync %0, %1;" : : "r"(barrier_id), "r"(num_threads) : "memory");
        ...
        auto store_O = [&]() {
            ...
            for (int i = 0; i < 64/16; ++i) { // 
                stsm_addrs[i] = &sO(stsm_row, (idx_in_warpgroup%32/16*8) + 16*i); /*warp内行偏移量（16行8列）*/ // 乱序地址打包，相当于一个 R-S mem 地址映射
            }
            bool s2g_pred = warp_idx%4 == 0 && elect_one_sync(); // 从每个wg里选一个来处理

            warpgroup_wait<0>();
            CUTE_UNROLL
            for (int tile_idx = 0; tile_idx < (D_V/2)/64; tile_idx += 1) { // D_V被分成了2块由两个wg分别处理，每次处理一行64个数据，因此是 tile_idx < (D_V/2)/64
                // Convert
                constexpr int NUM_ELEMS_EACH_TILE = B_H*64 / 128;   // 64: tile size, 128: warpgroup size 计算每个线程的寄存器中分配了多少数据
                bf16 cur_rOb[NUM_ELEMS_EACH_TILE]; // 声明一个register数组，存放缩后的结果
                CUTE_UNROLL
                for (int i = 0; i < NUM_ELEMS_EACH_TILE; ++i) {
                    cur_rOb[i] = (bf16)(rO(tile_idx*NUM_ELEMS_EACH_TILE + i) * scale_factors[i%4>=2]); // i%4>=2 d0,d1取0，d2,d3取1，也就是按行取
                }
                // R -> S
                CUTE_UNROLL
                for (int i = 0; i < 64/16; ++i) {
                    SM90_U32x4_STSM_N::copy( // U32：uint32， x4: 存4个
                        *reinterpret_cast<uint32_t*>(cur_rOb + i*8 + 0), // register指针访问，每组访问32位，也就是2个bf16，一共存4组
                        *reinterpret_cast<uint32_t*>(cur_rOb + i*8 + 2),
                        *reinterpret_cast<uint32_t*>(cur_rOb + i*8 + 4),
                        *reinterpret_cast<uint32_t*>(cur_rOb + i*8 + 6),
                        *reinterpret_cast<uint128_t*>(stsm_addrs[i] + tile_idx*(B_H*64)) // 合并成128字节一次写入。
                    );
                }
...
void KernelTemplate<D_QK, HAVE_TOPK_LENGTH>::run(const SparseAttnFwdParams &params) {
    ...
    cutlass::ClusterLaunchParams launch_params = {
        ...
        dim3(NUM_THREADS, 1, 1), // NUM_THREADS = 384
        dim3(1, 1, 1),
        smem_size,
        params.stream
    }; 
